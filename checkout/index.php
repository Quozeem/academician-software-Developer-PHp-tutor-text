...
?>
 <?php
 // insert customers details query
	...
	if(isset($_POST["submit"]))
{
	if(isset($_POST['submit'])){
        ...
        $createaccount=mysqli_real_escape_string($con,$_POST['createaccount']);
        ...
        $country=$_POST['country'];
                        ...
                        $_SESSION['STATES']=$state;
                         ...
                         $_SESSION['COUNTRY']=$country;
                          $_SESSION['LOCALGOV']=$LG;
		$address=htmlspecialchars($_POST['address']);
        $fulladdress=$address." ".$LG;
    ...
    //validate phone number
          ...
          date_default_timezone_set('Africa/Lagos');
			$date=date('M d, Y');
            $connected=new connection();
            $total=$connected->get_order_total();
            $email_exit=$connected->emailexist();
            $payment="";
              $tolly=$shipping+$total;
            $code = rand(00000000,99999999);
            //$shipping=mysqli_real_escape_string($con,$_POST['shipping']);
             $_SESSION["ordernumner"]=$code;
             $_SESSION["addresspayment"]=$address;
              $_SESSION["phonepayment"]=$phone;
              
              // validate shipping fee if empty
               if($shipping == ""){
                return "Shipping Fee is Empty";
              }
            
                  $file=$_POST['file'];
               //  $email_exit=$connected->emailexist();
            
                       while($rowed=mysqli_fetch_array($email_exit)){
    $mailler=$rowed['email'];
}if($mailler == $email){ 
         

  //if it's an exiting customers update his / her details
    $update=$connected->updateuser($name,$last_name,$email,$password,$createaccount,$date,$address,$phone);
}
         else{
            // if it's a new customers register is account
           $insert=$connected->Registration($name,$last_name,$email,$password,$createaccount,$date,$address,$phone);
             if(!$insert){
            ...
            //array function to insert all the purchased film with respect to individual prices and the numbet of film purchased
            foreach($file as $val=> $vales ) {  
                 $id=addslashes($_POST['id'][$i]);
                $totally =addslashes($_POST['totally'][$i]);
                 $brand=addslashes($_POST['brand'][$i]);
                 $price=addslashes($_POST['price'][$i]);
            //} 
            $size=addslashes($_POST['size'][$i]);
            $quantity=addslashes($_POST['qty'][$i]);
          // }
          $slect= mysqli_query($con,"SELECT * FROM joinall WHERE id='".$id."'");
          while($row=mysqli_fetch_array($slect)){
            $stockIn=$row['stockIn'];
          $stockqty=$row['qty'] + $quantity;
             $stockout=$stockIn-($stockqty);
          }
          // inventory function to kmwo the actual film left in the stock
          $update= mysqli_query($con,"UPDATE joinall SET stockout='$stockout',qty='$stockqty' WHERE id='".$id."'");
          if($update)
          {
            // customers details table
		$result = mysqli_query($con,"insert into customers_details(total,payment_method,country_id,zone_id,last_name,name,shipping,email,address,phone,size,orderNumber,orderimage,orderId,qty,brand,price,date,subtotal,payment)
        values('".$totally."','$codpayment','$country','$state','$last_name','$name','$shipping','$email','$fulladdress','$phone','".$size."','$code','".$vales."','".$id."','".$quantity."','".$brand."','".$price."','$date','$total','$payment')"); $i++;
        }
	if(!$result){
	   return ("Order Not Placed").mysqli_connect_error();
	}
    else{
                
      //  return "Order Placed Successfully";      //?id=<?php echo $row['id']  
              //}            //mail
              // to print receipt of order items
  echo "<script>  location.href='../orderReceived/?ordernumber=$code' </script>";
...
<?php include"../innersidebar.php";?>
		<header class="lte-page-header lte-bg-parallax-enabled">
		    <div class="container">
		    	<div class="lte-header-h1-wrapper"><h1 class="header">CHECKOUT</h1></div><ul class="breadcrumbs" typeof="BreadcrumbList" vocab="https://schema.org/"><!-- Breadcrumb NavXT 6.4.0 -->
...
<form name="checkout" method="post" id="paymentForm" class="checkout woocommerce-checkout" action="" enctype="multipart/form-data">
 ...
 //fetch customers details out if it's and existing customers
  if(isset($_SESSION['email'])) {
...
include "../includes/db.php";
 ...
 ?>
       ...
       <!--  <p class="form-row form-row-first validate-required" id="billing_address_2_field" data-priority="60">
          <label for="billing_address_1" class="">Apartment suite&nbsp;<abbr class="required" title="required">*</abbr></label><span class="woocommerce-input-wrapper">
...
<div class="woocommerce-additional-fields">
		...
		<div class="woocommerce-additional-fields__field-wrapper">
                            ...
                            <label for="order_comments" class="">Order notes&nbsp;<span class="optional">(optional)</span></label><span class="woocommerce-input-wrapper"><textarea name="order_comments" class="input-text " id="order_comments" placeholder="Notes about your order, e.g. special notes for delivery."  rows="2" cols="5"></textarea></span></p>					</div>
    ...
    <?php
    	if(is_array($_SESSION['cart'])){
            //	echo '<tr bgcolor="#FFFFFF" style="font-weight:bold"><td class="text-center">Serial</td><td class="text-center">Product</td><td class="text-center">Price
               // </td><td class="text-center">Quantity</td><td class="text-center">Sizes</td><td class="text-center">Amount</td></tr>';
				$max=count($_SESSION['cart']);
				for($i=0;$i<$max;$i++){
					$pid=$_SESSION['cart'][$i]['productid'];
					$q=$_SESSION['cart'][$i]['qty'];
...
function payWithPaystack(e) {
 ...
 <?php
	...
	echo '<script> alert("There are no items in your shopping cart!") 
